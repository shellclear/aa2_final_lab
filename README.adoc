...
- Create templates : deploy_osp_infra, ping_instances, deploy_base_config,
  ...
  template name
- Create workflows: Webapp_flow: start -> deploy_base_config -> 
  deploy_postgres|deploy_webapp|deploy_haproxy -> smoke_tests and oneclick
  _webapp: start -> deploy_osp_infra -> ping_instances -> Webapp_flow
